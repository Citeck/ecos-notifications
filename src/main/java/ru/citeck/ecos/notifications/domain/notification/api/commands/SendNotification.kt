...
import ru.citeck.ecos.notifications.domain.notification.service.NotificationStore
...
class SendNotificationCommandExecutor(
    ...
    private fun executeImpl(command: SendNotificationCommand): Any {
        return try {
            ...
            notificationStore.holdSuccess(command)

            return result
        } catch (e: Exception) {
            log.error("Failed execute notification command", e)

            notificationStore.holdError(command, e)
